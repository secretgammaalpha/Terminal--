...
class backup():
    ...
    def decompress():
        os.rename(f'{Install_Dir}/Terminal---2.02/{filename}.docx',f'{Install_Dir}/Terminal---2.02/{filename}.zip')
        time.sleep(3)
        shutil.unpack_archive(f'{Install_Dir}/Terminal---2.02/{filename}.zip','C:/Windows/Temp/temp_PasswdManager1.1/')
    def compress():
        backup.decompress()
        shutil.copy('Password.json', 'C:/Windows/Temp/temp_PasswdManager1.1/')
        shutil.make_archive(f'{Install_Dir}/Terminal---2.02/{filename}','zip','C:/Windows/Temp/temp_PasswdManager1.1/')
        os.rename(f'{Install_Dir}/Terminal---2.02/{filename}.zip',f'{Install_Dir}/Terminal---2.02/{filename}.docx')
